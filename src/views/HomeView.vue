            ...
            <div class="w-full tv:min-h-[2400px] relative z-20 bg-black" style="box-shadow: 0 -50px 40px 5px #000">
                ...
                <div class="sm:container px-4 flex items-center justify-center flex-wrap overflow-hidden">
                    <div class="flex items-center flex-col sm:px-12 px-8 sm:py-10 py-6 lg:mr-6 mr-8 mb-8 sm:space-y-8 space-y-4 sm:w-96 w-80 lg:h-80 rounded-tl-3xl rounded-br-3xl bg-m_amber-100 relative"
                         v-for="item in items" :key="item.id" v-scroll-reveal="{ origin: item.animate, distance: '50px', duration: 1000 }">
                        ...
                        <p class="font-inter font-normal text-center lg:text-base text-sm z-10">{{ $t(`home.cards.subtitle${item.id}`) }}</p>
